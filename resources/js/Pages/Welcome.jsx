            ...
            <div className="relative sm:flex sm:justify-center sm:items-center min-h-screen bg-dots-darker bg-center bg-gray-100 dark:bg-dots-lighter dark:bg-gray-900 selection:bg-red-500 selection:text-white">
                ...
                <div className="bg flex items-center">
                    <div className='w-full h-full pb-24'>
                        ...
                        <div className="row">
                            <div className='logo'>
                                <h1 className=' text-white text-lg md:text-xl lg:text-4xl'>{t('trader')}</h1>
                                <img src="img/logo.png" alt="Logo" />
                                <h1 className=' text-white text-lg md:text-xl lg:text-4xl'>{t('quiz')}</h1>
                            </div>
                        </div>
                            ...
                            <table className="w-full mb-12 md:mb-16 lg:mb-18">
                                <thead>
                                    <tr className="text-white">
                                        <th className="lg:px-2 py-3  text-center">#</th>
                                        <th className="lg:px-6 py-3 lg:text-lg font-bold uppercase text-center"> {t('userid')} </th>
                                        <th className="lg:px-6 py-3 lg:text-lg font-bold uppercase text-center">{t('score')}</th>
                                    </tr>
                                </thead>
                                <tbody style={{ display: scoreList.length > 0 ? 'ruby' : 'none' }}>
                                    {scoreList.map((user, index) => (
                                        <tr key={user.user_id} className="text-white">
                                            <td className="lg:px-2 py-4 text-sm  text-center">{index + 1}</td>
                                            <td className="lg:px-6 py-4 text-sm  text-center">{scoreList[index].user_name}</td>
                                            <td className="lg:px-6 py-4 text-sm  text-center">{scoreList[index].score}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                        ...
                        <div className={`purpleStyle alert ${showShareAlert ? 'show' : ''}`}>
                            <h2 className='text-lg md:text-xl lg:text-4xl'> {t('introduce_friends')}</h2>
                            ...
                            {props.user ? (
                                <div>
                                    <p className='text-white'> {t('invite_code')} : {props.invite_code} </p>
                                    <button id='copyClipboard' className='shareDiv' onClick={handleCopyClick}>
                                        ...
                                        <input type="text" name="telegramURL" readOnly defaultValue={window.location.href + 'register:' + props.invite_code} tabIndex="0" />
                                    </button>
                                </div>
                            ) : (
                                <div className='flex flex-col justify-center items-end'>
                                    <p className='text-sm md:text-xl lg:text-2xl'> {t('please_signin')} </p>
                                    <Link href={route('login')} style={{ textDecoration: 'none' }}>
                                        <button className='loginButton'>{t('login')}</button>
                                    </Link>
                                </div>
